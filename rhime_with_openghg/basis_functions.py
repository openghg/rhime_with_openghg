# ------------------------------------------------------------------------------
# About:
#   Algorithms for creating basis functions (scaling regions) as used
#   in the RHIME inverse model. 
#   Algorithms currently include the: QuadTree algorithm, and the "weighted"
#   algorithm. The latter of which includes a land-sea split. The number of 
#   basis functions is specified by the user as an input. 
# ------------------------------------------------------------------------------
# TO-DO
#   [] Option to specify country split in addition to land-sea
#   [] Other algorithms to include?
#   [] Reformat the netcdf output to be CF compliant
...
def quadtreebasisfunction(emissions_name, fp_all, sites, 
    ...
    Returns:
        If outputdir is None, then returns a Temp directory. 
        The new basis function is saved in this Temp directory.
        If outputdir is not None, then does not return anything but saves 
        the basis function in outputdir.
    -----------------------------------
    """
    if abs_flux:
...
# Optimise bucket value to number of desired regions 
...
def optimize_nregions(bucket, grid, nregion, tol):
    """
    Optimise bucket value to obtain nregion basis functions
    within +/- tol.
    """
