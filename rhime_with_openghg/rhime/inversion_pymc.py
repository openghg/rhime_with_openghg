...
#
#
#        #model_error_scale = np.abs(pm.math.dot(hx,x)) * sig[sites, sigma_freq_index]
#        model_error_scale = pm.math.abs(pm.math.concatenate(hx_dot_x)) * sig[sites, sigma_freq_index]
#        model_error_min = (sig[sites, sigma_freq_index]**0) * model_error_min_values[species.lower()]
#        model_error = pm.math.maximum(model_error_scale, model_error_min)
#
#        epsilon = pm.math.sqrt(error**2 + model_error**2)
        ...
        x_sample = pm.math.concatenate(x_conc)        

        x_conc.append(xbc)
        #step1 = pm.NUTS(vars=[x_sample,xbc])
