...
#     lat = lat.sel(lat=slice(latmin_ds,latmax_ds))
    ...
    if plot_y_timeseries == True:
        ...
        if combine_timeseries and len(ds_list) > 1:
            ...
            process.plot_timeseries(ds_combined, 
                                    fig_text=None, 
                                    ylim=None, 
                                    out_filename=y_out_filename)
        else:
            for i,ds in enumerate(ds_list):
                ...
                process.plot_timeseries(ds, 
                                        fig_text=None, 
                                        ylim=None, 
                                        out_filename=y_out_filename_n)
    ...
    if calc_country == True:
        cntrymean_arr, cntry68_arr, cntry95_arr, cntryprior_arr \
         = process.country_emissions_mult(ds_list, 
                                          species,  
                                          domain, 
    ...
    if plot_countryemissions == True and len(dates)>1:
        for ii,cntry in enumerate(countries_to_plot):
            ...
            process.plot_country_timeseries(cntrymean_arr[:,cntry_ind], 
                                            cntryCI_arr[:,cntry_ind,:],
                                            cntryprior_arr[:,cntry_ind], 
                                            dates, 
                                            country_label=cntry, 
                                            units=units, 
                                            figsize = (7,3))
